 ...
 *
 * Returns loan accounts that still have payable principal on the requested
 * repayment date.
 *
 * Query params:
 ...
 *     currentUnpaidPeriod, currentPeriodPaid }] }
...
export async function GET(request: Request) {
  ...
  try {
    const household = await getHouseholdScope();
    const hidFilter = household.hidFilter ?? {};
