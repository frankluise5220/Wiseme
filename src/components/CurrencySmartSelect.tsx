...
/** The 19 built-in system currencies ship with the frontend bundle — render
 * them synchronously without waiting for the API (which only adds the small
 * user-approved currency list on top). */
function buildSystemOnlyList(): CurrencyEntry[] {
  return CURRENCY_OPTIONS.map(({ value: code }) => ({
    code,
    nameZh: "",
    nameEn: "",
    countryZh: null,
...
async function fetchCurrencies(force = false): Promise<CurrencyEntry[]> {
  ...
  const promise = fetch("/api/v1/currencies?includeSystem=true", { cache: "no-store" })
    ...
    .catch(() => {
      ...
      const fallback = buildSystemOnlyList();
      CACHE.list = fallback;
      CACHE.updatedAt = Date.now();
      return fallback;
    });
...
export function CurrencySmartSelect({
  ...
  // Seed synchronously with the built-in system currencies (or the cached
  // merged list) so the dropdown is never blank while the API is in flight;
  // the async refresh below only appends user-approved currencies.
  const [currencies, setCurrencies] = useState<CurrencyEntry[]>(() => CACHE.list ?? buildSystemOnlyList());
  ...
  useEffect(() => {
    if (CACHE.list) {
      // Show the cached list immediately; refresh silently when stale so the
      // selector never blocks on the database.
      setCurrencies(CACHE.list);
      if (Date.now() - CACHE.updatedAt >= TTL_MS) void reload();
    } else {
      void reload();
    }
  }, [reload]);
